...
class SemanticCopyandPaste(A.DualTransform):
    ...
    def __init__(self,
        ...
        if shift_x_limit is not None:
            assert type(shift_x_limit) == list and type(shift_y_limit) == list and type(rotate_limit) == list and type(
                scale) == list
            assert abs(shift_x_limit[0]) <= 1 and abs(shift_y_limit[0]) <= 1 and abs(rotate_limit[0]) <= 1 and abs(
                rotate_limit[1]) <= 1 and 0 <= scale[0] <= scale[1] and scale[
                       1] >= 1, 'The range for shift_x/y_limit and rotate is [-1 to 1], and [0 to 1] for scale'
        ...
        for i, mName in enumerate(self.masks):
            ...
            for j in range(self.nClass):
                if self.target_class_in_image(c_mask, j):  # If class pixel > self.threshold
        ...
        if self.auto_weights:
            ...
            if not class_weights:
                ...
                assert len(
                    class_weights) == nClass, "class_weights' length != nClass, nClass should also include the background class."
    ...
    def imgTransform(self, image, offset_x_limit, offset_y_limit):
        """
            Args:
                ...
                offset_x_limit: x-axis shift limit [-1,1]
                offset_y_limit: y-axis shift limit [-1,1]
        """
        ...
        self._transformation_matrix = cv2.getRotationMatrix2D((self.imgRow // 2, self.imgCol // 2), rotate_deg,
                                                              scale_coef)
